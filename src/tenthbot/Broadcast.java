    ...
    MeleeUnit[] getRangers(){
        ...
        for (Robot r : utils.robotsInVision){
            ...
            if (canReadSignal(r)){
                ...
                ans[cont] = new MeleeUnit(x,y, Constants.PROPHET, r.x, r.y);
                ++cont;
            }
        }
        return ans;
        ...
        for (Robot r : utils.robotsInVision){
            ...
            if (canReadSignal(r)){
                ...
                if (mes == CLOSEST_CRUSADER) {
                    int y = r.signal % Constants.maxMapSize;
                    int x = (r.signal / Constants.maxMapSize) % Constants.maxMapSize;
                    ans[cont] = new MeleeUnit(x, y, Constants.CRUSADER, r.x, r.y);
                    ++cont;
                }
                if (mes == CLOSEST_PREACHER) {
                    int y = r.signal % Constants.maxMapSize;
                    int x = (r.signal / Constants.maxMapSize) % Constants.maxMapSize;
                    ans[cont] = new MeleeUnit(x, y, Constants.PREACHER, r.x, r.y);
                    ++cont;
                }
            }
        }
