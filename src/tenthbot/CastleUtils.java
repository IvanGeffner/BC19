...
public class CastleUtils {
    ...
    ProphetLoc getBestProphetLoc(Integer dir){
        ...
        ProphetLoc bestProphetLoc = null;
        int distFound = -1;
        distM[myX][myY] = 1;
        ...
        while (!queue.isEmpty()) {
            ...
            if (distFound >= 0 && lastDist > 2*distFound) break;
            if (lastDist > 1 && (lastX + lastY)%2 == Constants.MOD2 && !myRobot.karboniteMap[lastY][lastX] && !myRobot.fuelMap[lastY][lastX]){
                if (isFree(lastX, lastY)){
                    if (distFound < 0 || distFound > lastDist) distFound = lastDist;
                    ProphetLoc newLoc = new ProphetLoc(dirs[lastX][lastY], new Location(lastX, lastY));
                    if (newLoc.isBetterThan(bestProphetLoc)) bestProphetLoc = newLoc;
                }
            }
    ...
    void generateObjectiveList(){
        ...
        for (int i = 0; i < myCastles.length; ++i){
            if (myCastles[i].wellDefined()){
                ...
                queue.add((myCastles[i].x << Constants.SHIFT) | myCastles[i].y);
            }
        }
        ...
        while (!queue.isEmpty()){
            ...
            int lastX = (last >>> Constants.SHIFT) & Constants.BASE;
            int lastY = last & Constants.BASE;
            int lastDist = mapDist[lastX][lastY];
            int castle = closestCastle[lastX][lastY];
            if (myRobot.fuelMap[lastY][lastX]){
                aux.add(new Objective(Constants.OBJ_FUEL, lastDist-1, lastX, lastY, dir[lastX][lastY],castle, fuelM[lastX][lastY]));
                ++fuelCont;
            }
            if (myRobot.karboniteMap[lastY][lastX]){
                aux.add(new Objective(Constants.OBJ_KARBO, lastDist-1, lastX, lastY, dir[lastX][lastY],castle, fuelM[lastX][lastY]));
                ++karboCont;
            }
            int limit = Constants.rad4Index;
            if (lastDist == 1) limit = Constants.rad2Index;
            for (int i = 0; i < limit; ++i){
                int newX = lastX + Constants.X[i];
                int newY = lastY + Constants.Y[i];
                int newFuel = fuelM[lastX][lastY] + Constants.Steplength[i];
                if (utils.isInMap(newX, newY) && utils.isAccessible(newX, newY)){
                    if (mapDist[newX][newY] == 0){
                        ...
                        else dir[newX][newY] = dir[lastX][lastY];
                    }
    ...
    int getBestIndex(){
        ...
        boolean betterKarbo = objectives[karbo[karboObjectiveIndex]].isBetterThan(objectives[fuel[fuelObjectiveIndex]]);
        if (betterKarbo){
            return karbo[karboObjectiveIndex];
        }
        if (karboObjectiveIndex <= fuelObjectiveIndex){
            return karbo[karboObjectiveIndex];
        }
        if (!betterKarbo){
            return fuel[fuelObjectiveIndex];
    ...
    Integer shouldBuildPilgrim(){
        ...
        /*for (int objective = 0; objective < objectives.length; ++objective) {
            if (!finalAttack.alreadyCalled && objectives[objective].isNearEnemy()) continue;
            int occ = isOccupied(objective);
            if (occ == Constants.FREE) {
                totalCostKarbo += Constants.karboCosts[Constants.PILGRIM];
                totalCostFuel += Constants.fuelCosts[Constants.PILGRIM];
    ...
    class Objective{
        int type, dist, x, y, dir, castle, myFuel;
        Double value = null, standardValue = null;
        ...
        /*double value(){
            ...
            value += 1.0/this.myFuel;
            value = -value;
            return value;
        }*/

        //BUILD UNITS AGAINST PILGRIMS OR CHURCHES

        double standardValue(){
            if (standardValue != null) return standardValue;
            standardValue = 0.0;
            double distToCastle = mapDist[x][y];
            Location symLoc = finalAttack.symmetry.getSymmetric(x,y);
            double distToEnemyCastle = mapDist[symLoc.x][symLoc.y];
            if (distToCastle > 0 && distToEnemyCastle > 0) {
                standardValue = (distToCastle) / (distToCastle + distToEnemyCastle);
                if (standardValue > 0.5) standardValue = 2.0 - 3.0*standardValue;
                if (standardValue < 0) standardValue = 0.0;
            }
        ...
        double value(){
            if (value != null) return value;
            value = (standardValue() + 0.25);
            value *= value;
            value /= ((this.dist + 1.0));
            if (dist <= 3) value += Constants.INF;
            value = -value;
            if (type == Constants.OBJ_KARBO) value *= 1.2;
            return value;
        }
        ...
        void print(){
            myRobot.log(this.type + " " + this.dist + " " + this.myFuel + " " + this.x + " " + this.y + " " + this.dir + " " + this.castle + " " + value() + " " + standardValue());
        }
    }
    ...
    class ProphetLoc{
        ...
        int dxAbs, dyAbs, value;

        public ProphetLoc(int dirInitial, Location target){
            this.dirInitial = dirInitial;
            this.target = target;
            dxAbs = target.x - myRobot.me.x;
            dyAbs = target.y - myRobot.me.y;
            if (dxAbs < 0) dxAbs = -dxAbs;
            if (dyAbs < 0) dyAbs = -dyAbs;
            int xVal = dxAbs*finalAttack.symmetry.xMult, yVal = dyAbs*finalAttack.symmetry.yMult;
            value = xVal;
            if (yVal > xVal) value = yVal;
        ...
        boolean isBetterThan(ProphetLoc loc){
