...
public class CastleUtils {
    ...
    void checkBuildPreacher(){
        if (nextTurnAction != null) return;
        Integer initialDir = getBestDir();
        if (initialDir == null) return;
        if (castleIndex != 0) return;
        DefenseMechanismAction dir = defenseMechanism.buildDefenseUnit(Constants.PREACHER, false);
        if (dir != null) nextTurnAction = myRobot.buildUnit(Constants.PREACHER, Constants.X[dir.dir], Constants.Y[dir.dir]);
    }

    Integer getBestDir(){
        Integer bestDir = null;
        int bestDist = 0;
        for (int i = 0; i < Constants.rad2Index; ++i){
            int newX = myRobot.me.x + Constants.X[i];
            int newY = myRobot.me.y + Constants.Y[i];
            if (!utils.isEmptySpaceAbsolute(newX, newY)) continue;
            Location symmetric = finalAttack.symmetry.getSymmetric(myRobot.me.x, myRobot.me.y);
            int d = utils.distance(newX, newY, symmetric.x, symmetric.y);
    ...
    void checkPilgrim(){
        ...
        if (objective != null && objectives[objective].fuel <= Constants.MAX_FUEL_LOST) createPilgrim(objective);
    }
