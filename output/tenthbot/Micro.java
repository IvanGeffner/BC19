...
public class Micro {
    ...
    void generateMicroArray(){
        ...
        if (broadcast != null){
            MeleeUnit[] enemyRangers = broadcast.getRangers();
            for (MeleeUnit loc : enemyRangers) {
                if (loc == null) break;
    ...
    class MicroInfo{
        ...
        void updateRanger(MeleeUnit loc){
            ...
            int d2 = utils.distance(loc.signalOriginX, loc.signalOriginY, x, y);
            if (d2 <= Constants.CLOSEST_PROPHET_RANGE && minRange > d) minRange = d;
            if (d >= Constants.minRange[myRobot.me.unit] && d <= Constants.dangerRange[myRobot.me.unit]) canShoot = true;
        }
