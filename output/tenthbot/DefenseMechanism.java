...
public class DefenseMechanism {
    ...
    DefenseMechanismAction defenseAction(boolean safe){
        ...
        mageNearby = 0;
        int closestEnemy = Constants.INF;
        boolean prophetNearby = false;
            ...
            if (myRobot.isVisible(r)){
                if (r.team != myRobot.me.team){
                    ...
                    if (r.unit > Constants.PILGRIM && utils.distance(myRobot.me.x, myRobot.me.y, r.x, r.y) <= Constants.MAX_DIST_TROOPS){
                        if (r.unit == Constants.PROPHET && utils.distance(myRobot.me.x, myRobot.me.y, r.x, r.y) <= Constants.MAX_DISTANCE_PROPHET) prophetNearby = true;
                        ++totalTroops;
                        ++myUnits[r.unit];
                        ++totalUnits;
        ...
        if (enemyUnits[Constants.CRUSADER]/2 + enemyUnits[Constants.PREACHER] > myMelees){
            //myRobot.log("Need melees!");
            return buildDefenseUnit(Constants.PREACHER, safe);
        }
        int myRangeds = myUnits[Constants.PROPHET] - Constants.DIFF_RANGERS;
        if (myRangeds < 0) myRangeds = 0;
        if (enemyUnits[Constants.PROPHET] + enemyUnits[Constants.CHURCH] > myRangeds){
            //myRobot.log("Need range!");
            if (closestEnemy <= Constants.MAX_DISTANCE_CLOSE) {
                return buildDefenseUnit(Constants.PREACHER, safe);
            }
            return buildDefenseUnit(Constants.PROPHET, safe);
        }
        if (enemyUnits[Constants.PILGRIM] + enemyUnits[Constants.CHURCH] > 0 && !prophetNearby){
            //myRobot.log("Need  killing pilgrims!");
            if (closestEnemy <= Constants.MAX_DISTANCE_CLOSE) {
                return buildDefenseUnit(Constants.PREACHER, safe);
            }
            return buildDefenseUnit(Constants.PROPHET, safe);
