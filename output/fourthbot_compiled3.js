    ...
    var Church = (function (_super) {
        ...
        Church.prototype.turn = function () {
            ...
            if (act != null)
                return act;
            if (this.rich()) {
                return this.tryBuildProphet();
            }
            return null;
        };
        Church.prototype.rich = function () {
        ...
        Church.prototype.tryBuildProphet = function () {
            for (var i = 0; i < bc19.Constants.rad2Index; ++i) {
                if (this.utils.isEmptySpace(bc19.Constants.X_$LI$()[i], bc19.Constants.Y_$LI$()[i]))
                    return this.myRobot.buildUnit(bc19.Constants.PROPHET, bc19.Constants.X_$LI$()[i], bc19.Constants.Y_$LI$()[i]);
            }
            ;
            return null;
        };
        ...
        Prophet.prototype.moveToBestLoc = function () {
            ...
            while ((!(queue.length == 0))) {
                ...
                if ((last.x + last.y) % 2 === this.mod2 && !this.myRobot.karboniteMap[last.y][last.x] && !this.myRobot.fuelMap[last.y][last.x]) {
                    var d = this.distFromObjective[last.x][last.y];
                    if (d < bestDist) {
                        bestDist = d;
        ...
        Prophet.prototype.getAttackAction = function () {
            ...
            for (var index130 = 0; index130 < this.utils.robotsInVision.length; index130++) {
                var r = this.utils.robotsInVision[index130];
                {
                    ...
                    if (!this.inRange(r))
                        continue;
                    if (bestRobot == null || r.health < bestRobot.health)
                        bestRobot = r;
                }
            }
            if (bestRobot != null) {
