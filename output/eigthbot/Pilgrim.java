    ...
    void goToDestination(){
        ...
        if (targetCode == MINING_TARGET){
            if (myX == destination.x && myY == destination.y){
                if (myRobot.me.karbonite < Constants.MAX_KARBO_PILGRIM_RICH && myRobot.me.karbonite < Constants.MAX_FUEL_PILGRIM) actionToPerform = myRobot.mine();
                return;
            }
        ...
        while (!queue.isEmpty()) {
            ...
            if (isDestination(last)){
                if (micro.isSafe(dirs[last.x][last.y])) actionToPerform = myRobot.move(Constants.X[dirs[last.x][last.y]], Constants.Y[dirs[last.x][last.y]]);
                return;
            }
